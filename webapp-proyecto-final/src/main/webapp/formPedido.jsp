...
<form action="${pageContext.request.contextPath}/pedidos/form" method="post">
    ...
    <label for="nombreCliente" class="col-form-label col-sm-3 text-center">Nombre cliente</label>
    <label for="direccionCliente" class="col-form-label col-sm-3 text-center">Dirección cliente</label>
...
<%--                </c:forEach>--%>
        ...
        <c:if test="${errores != null && not empty errores.estado}">
            ...
            <input class="col-sm-3" type="text" name="nombreCliente" id="nombreCliente" value="${pedido.nombreCliente}" class="form-control">
        <c:if test="${errores != null && errores.containsKey('nombreCliente')}">
            <div style="color:red;">${errores.nombreCliente}</div>
        </c:if>

            <input class="col-sm-3" type="text" name="direccionCliente" id="direccionCliente" value="${pedido.direccionCliente}" class="form-control">
...
<%--    </div>--%>
    ...
    <div class="row mb-2 mt-2 col-12">
            <input class="btn btn-primary d-inline-block col-lg-2" type="submit" value="${pedido.id!=null && pedido.id>0? "Editar": "Agregar ítem"}">
    </div>
<%--    <input type="hidden" name="id" value="${pedido.id}">--%>
    <div class="row mt-2">
          <input class="btn btn-primary flex-end ml-1 col-lg-2" type="submit" value="${pedido.id!=null && pedido.id>0? "Editar": "Crear pedido"}">
    </div>
</form>
